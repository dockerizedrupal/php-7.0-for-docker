...
DOCKER_COMPOSE_FILE="${BATS_TEST_DIRNAME}/php-7.0_drupal_8_drush_8.yml"
...
@test "php-7.0: drupal 8: phpcs: phpcompatibility" {
  run docker exec "$(container)" /bin/su - root -mc "phpcs -i | grep 'PHPCompatibility'"

  [ "${status}" -eq 0 ]
}

@test "php-7.0: drupal 8: phpcs: drupal" {
