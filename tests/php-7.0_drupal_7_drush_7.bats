...
DOCKER_COMPOSE_FILE="${BATS_TEST_DIRNAME}/php-7.0_drupal_7_drush_7.yml"
...
@test "php-7.0: drupal 7: phpcs: phpcompatibility" {
  run docker exec "$(container)" /bin/su - root -mc "phpcs -i | grep 'PHPCompatibility'"

  [ "${status}" -eq 0 ]
}

@test "php-7.0: drupal 7: phpcs: drupal" {
